 ...
 */
export function rescaleAndRound({ operands, divisor, rounding }) {
    if (rounding !== "half_up") {
 ...
 */
export function runDerivation(derivation, resolveOperand) {
    const family = PRIMITIVES[derivation.primitive];
    if (!family) {
        throw new Error(`unknown derivation primitive "${derivation.primitive}"; supported: ${Object.keys(PRIMITIVES).join(", ")}`);
    }
