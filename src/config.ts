 ...
 */
export function resolveSaltDir(explicit?: string): string {
  if (explicit) return explicit;
  if (process.env.VCC_SALT_DIR) return process.env.VCC_SALT_DIR;
  if (process.env.XDG_DATA_HOME) return path.join(xdgDataHome(), "vcc", "private");
  return path.join(os.homedir(), ".vcc", "private");
}

/** Resolve packageDir following the same precedence as resolveSaltDir. */
export function resolvePackageDir(explicit?: string): string {
  if (explicit) return explicit;
  if (process.env.VCC_PACKAGE_DIR) return process.env.VCC_PACKAGE_DIR;
  if (process.env.XDG_DATA_HOME) return path.join(xdgDataHome(), "vcc", "packages");
