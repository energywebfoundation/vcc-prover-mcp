 ...
 */
export function encodeExact(decimal: string | number, places: number): EncodeResult {
  ...
  if (fracPart.length > places) {
    return {
      ok: false,
      reason:
        `"${text}" has ${fracPart.length} decimal places but the pinned precision is ${places}. ` +
 ...
 */
export function decodeToDecimal(value: bigint | number | string, places: number): string {
