...
function paramsByKind(params: SchemaParam[], kind: string): SchemaParam[] {
...
export async function prove({
  ...
  let resolvedCircuitJson: any = circuitJson;
  ...
  let circuitRawBytes: Buffer | null = circuitBytes ? Buffer.from(circuitBytes) : null;
  ...
  const hasVkInRecipe = Boolean(
    recipe.verification_package?.vk_b64 || recipe.verification_package?.vk
  );
  ...
  const encodedValues = new Map<string, bigint>();
  const rawInputsRecord: Record<string, string | number> = {};
  ...
  for (const p of paramsByKind(schema.params, "private_input")) {
    ...
    if (!enc.ok) return { ok: false, reason: `${p.name}: ${enc.reason}` };
  ...
  for (const p of paramsByKind(schema.params, "coefficient")) {
    ...
    if (supplied !== null && supplied !== undefined) {
      ...
      const pinnedEncoded =
        (recipe as any).factor?.value_encoded ??
        (recipe as any).factors?.[p.name] ??
        (recipe as any).factors?.emission_factor;
  ...
  const proveRes = await proveCircuit(
    effectiveCircuitDir,
    circuitName,
    inputsMap,
    tag,
  ...
  if (!proveRes.ok) return proveRes;
  ...
  const publicSignals: Record<string, string> = {};
  const publicSignalsOrdered: string[] = [];
  ...
  const encodedInputsRecord: Record<string, string> = {};
  for (const [k, v] of encodedValues.entries()) {
    encodedInputsRecord[k] = v.toString();
  }
  const saltsRecord: Record<string, string> = {};
  ...
  const privateAudit: PrivateAuditPack = {
    formula: {
      id: recipe.formula.id,
      version: recipe.formula.version
    },
